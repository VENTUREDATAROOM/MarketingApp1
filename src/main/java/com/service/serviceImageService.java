...
public class serviceImageService {
	...
	public serviceImage addServiceImage(MultipartFile file, String code) throws IOException {
		...
		try {
			...
			//byte[] byteImageIconData=imageIcon.getBytes();
			//serviceImageData.setImageIcon(byteImageIconData);
		} catch (Exception e) {
			e.printStackTrace();
		}
...
//        });
		...
		for (serviceImage image : serviceImageData) {
			...
			//String encodedImage=byteImageToBase64(image.getImageIcon());
			//newResponseImage.setEncodedImageIcon(encodedImage);
